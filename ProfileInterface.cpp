...
bool ProfileInterface::importSnapmaticPicture(SnapmaticPicture *picture, bool warn)
{
    ...
    if (picFileName.left(4) != "PRDR")
    {
        if (warn) QMessageBox::warning(this, tr("Import..."), tr("Failed to import the Snapmatic picture, file not begin with PRDR or end with .r5e"));
        return false;
    }
    else if (QFile::exists(profileFolder % "/" % adjustedFileName) || QFile::exists(profileFolder % "/" % adjustedFileName % ".hidden"))
    {
        ...
        if (warn)
        {
            ...
            if (uchoice == QMessageBox::Yes)
            {
                ...
                {
                    snapmaticProperties.uid = getRandomUid();
                    fExists = QFile::exists(profileFolder % "/PRDR3" % QString::number(snapmaticProperties.uid));
                    fExistsBackup = QFile::exists(profileFolder % "/PRDR3" % QString::number(snapmaticProperties.uid) % ".bak");
                    fExistsHidden = QFile::exists(profileFolder % "/PRDR3" % QString::number(snapmaticProperties.uid) % ".hidden");
                    cEnough++;
                }
...
void ProfileInterface::contextMenuTriggeredPIC(QContextMenuEvent *ev)
{
    ...
    QAction *mapViewer = editMenu.addAction(PictureDialog::tr("Open &Map Viewer..."), picWidget, SLOT(openMapViewer()));
